...
def checkEmpty(lagerID, lagerPLZ):
    ...
    zipcodeInputEnter = driver.find_element_by_xpath(
        '/html/body/div[1]/div[2]/div/div/div/button')
    ...
    top = pd.read_csv(
        "analytics.csv",
        header=0,
        usecols=['Suchbegriff', 'WarehouseId'],
        sep=';')
    ...
    for i in top['Suchbegriff']:
        ...
        searchIcon = driver.find_element_by_xpath(
            '/html/body/div[1]/header/div[3]/div/div[2]/div[1]/div/div')
        searchTerm = driver.find_element_by_xpath('//*[@id="searchTerm"]')
        searchTermSubmit = driver.find_element_by_xpath(
            '//*[@id="search-form-header"]/div[1]/button')
        sleep(2)
        searchIcon.click()
        sleep(2)
...
warehouse = pd.read_csv(
    'warehouse.csv', header=0, usecols=['LagerId', 'LagerPLZ'], sep=',')
